...
public class CustomOidcUserService extends OidcUserService {
    ...
    private OidcUserDTO getUsuarioDTO(OidcUserRequest request, OidcUser oidcUser) {
        ...
        try {
            ...
            if (atributos.get("realm_access") instanceof Map<?, ?> realms
                ...
                OidcUserDTO usuario = new OidcUserDTO();
                List<SimpleGrantedAuthority> authoritys;
                authoritys = ((List<String>) rolesObject).stream().map(SimpleGrantedAuthority::new)
                        .toList();
                usuario.setAuthorities(authoritys);
                usuario.setClaims(oidcUser.getClaims());
                usuario.setOidcIdToken(oidcUser.getIdToken());
                usuario.setAcessToken(request.getAccessToken());
                return usuario;
            }
